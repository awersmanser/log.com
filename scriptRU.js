...
window.onload = function () {
    ...
    setTimeout(() => {
        ...
        if (cookieAccpetShow) { callNotification('Уведомление: cookie файлы', 'Этот сайт использует cookie файлы, вы согласны?', 'ДА'); document.cookie = 'cookieAccpetShow=false; secure' }
    }, 500);
...
function start() {
    ...
    if (authCheck().includes('Login: true') & authCheck().includes('password: true')) {
        ...
        if (show) {
            select = 0;
            callNotification('Уведомление', 'Случайные 10 чисел от 0 до 9 генерируются и используются для определения светимости звезды. Создаёт эффект огня.', 'ОК');
            show = false;
            overflow = false;
        }
    } else { authAttention() }
